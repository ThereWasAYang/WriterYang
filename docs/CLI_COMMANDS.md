...
novel revision-session cancel <revision_session_id> --path ./qingdeng-inn
```

只可对已认可且仍为 canonical 最新章节的正文创建局部修订。`blocks` 返回稳定 block 编号、类型、预览和 hash；`start` 冻结授权范围；`run` 生成 structured patch、合成完整 candidate 并重新 Audit/构建 state projection；`accept` 事务性提交同一 candidate；`cancel` 将 Session 置为终态，并从当前 acceptance 的 immutable artifact 原子恢复 `polished.md`、`audit.json` 和 `state_update_proposal.json`。Creation Session 不再接受 `--chapter` 或 `--segments`。
