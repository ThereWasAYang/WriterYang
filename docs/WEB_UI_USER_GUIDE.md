...
| 项目检查 | 运行 validation，并顺带检查 schema 是否需要迁移。 | 结果会显示在“项目检查结果”；完整 JSON 详情会写入“运行日志 / 项目文件”的“章节文件查看”。如果需要迁移，会显示“一键迁移项目 Schema”。 |
| 项目迁移 | 为旧项目补齐 `schema_version` 或 timeline 双轨字段。 | 只有项目检查发现需要迁移时才显示按钮；执行时会使用项目锁和备份。 |
...
| 区域 | 用途 | 使用方法 |
| --- | --- | --- |
| 项目搜索 | 搜索角色、地点、物品、时间线事件和章节正文。 | 输入关键词后点击“搜索”。默认使用 FTS；只有勾选“使用 embedding 语义检索”才会调用外部 embedding API。 |
