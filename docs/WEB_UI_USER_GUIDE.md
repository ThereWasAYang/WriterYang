...
1. 在“新项目父目录”输入一个本地文件夹路径，例如 `~/WriterYang` 或 `/Users/yang/Novels`。Web UI 会在这个父目录下按书名创建项目子目录。
...
页面顶部是主导航，分为“主页”“创作工作台”“文风设置”“小说状态管理”“模型与检索配置”“运行日志 / 项目文件”。运行环境详情显示在主页的信息框中，不会常驻在每个页面顶部。多数长任务会显示已用时和最近操作；真实 API 写作、审核和索引可能需要等待。Session 写作期间，“取消当前 Session 任务”和刷新类按钮仍可用。
...
| 页面 | 用途 | 使用方法 |
| --- | --- | --- |
| 主页 | 打开/初始化项目、查看运行环境、项目检查、查看项目摘要和下一步提示。 | 新用户先在这里打开项目；需要确认 Python 环境、Web UI 端口或启动器端口时看运行环境信息框；每次手动编辑 memory 后点“项目检查”。 |
...
创作工作台是普通作者最常用的页面。页面顶部有固定的“创作输入”横栏；滚动查看“Session 主流程”“创作中设定调整”或右侧状态面板时，“聊天 / 指令”仍会停留在当前画面内，方便随时补充长段创作要求。
...
“文风设置”页编辑长期生效的 `memory/style_guide.md`。页面会保留现有 Markdown 内容；如果文件缺失，会载入默认模板。点击“保存文风设置”后，旧文件会先备份，再写入新内容。后续创建大纲、写初稿、润色和审核都会读取这个文件。

如果希望先让模型起草，可以在“AI 生成文风草稿”里输入自然语言方向，例如“结合沧月、小椴、古龙、步非烟的风格”。系统会调用 `style_guide` Agent 输出结构化 JSON，再由本地渲染为 Markdown 并填入编辑器；此时页面会显示未保存修改，只有点击“保存文风设置”才会写入 `memory/style_guide.md`。生成时会参考当前项目和已有文风设置；如果编辑器已有未保存内容，页面会先要求确认再替换。

文风生成只总结高层风格特征，例如叙事气质、语言密度、对白、节奏和禁用项，不会引用原文，也不应要求模型复刻或冒充任何具体作者。生成结果仍建议人工审阅后再保存。

适合写在这里的内容包括整体风格、叙事视角、语言密度、对白要求、节奏、禁用项、示例段落和修订备注。如果只是某一章的临时要求，继续写在“创作工作台”的“聊天 / 指令”里，不要写入长期文风设置。
...
| 状态和时间线 | 查看角色、物品、地点状态和 timeline。 | 用于检查状态流转、物品持有人、地点变化和时间线事件。 |
...
1. 点击“Canon 建议”。
2. 生成完成后，“Canon proposal” 会填入文件路径，“Canon proposal 预览”会在工作台内显示 proposal JSON。
3. 读一遍预览内容，确认人物、地点、物品、世界规则大方向是否合适；需要排查结构化返回时，再去“运行日志 / 项目文件”查看详情。
4. 确认后点击“应用 Canon proposal”。
5. 应用后，预览会切换到本次应用保存的 proposal 快照路径。
6. 再点击“项目检查”。
...
1. 在工作台右侧“大纲预览”查看 `outline_proposal.md`。
...
7. 如果是尚未认可的 Session 正文主观意见，在“聊天 / 指令”写清楚，再点击“按用户意见修订内容”。系统会先显示“本次修改路由”：剧情结构变化会回到大纲/章节计划，人物刻画、节奏、风格问题会重写正文。
8. 如果要修改已经认可的最新章节，使用独立的“Accepted 章节局部修订”面板。先加载 block 并确认范围；该路径会生成 `SegmentPatch`，保证范围外正文逐字节不变，并重新完成 Audit、State Proposal 和事务性认可。
...
状态和时间线页可以帮助你检查角色位置、物品持有人、地点状态和事件顺序。
...
在主页导出区选择导出参数后，点击“导出 Markdown”或“导出 DOCX”。系统只导出带 fresh `acceptance.json` 的 `accepted.md`。

如果某一章没有认可，导出时会跳过并给出 warning；如果已认可章节的正文或 lineage 被手工修改，导出会直接拒绝 stale 内容。长篇创作时建议每个 Session 完成后先整体认可，再导出。
