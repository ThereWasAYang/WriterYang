...
from novel.core.schemas import ExportManifest, ExportRecord, ProjectConfig
...
def export_markdown(options: MarkdownExportOptions) -> MarkdownExportResult:
    ...
    output_path.write_text(render_markdown_export(title, chapters), encoding="utf-8")
...
def render_markdown_export(title: str, chapters: list[ExportedChapter]) -> str:
    lines = [f"# {title}", ""]
