...
SCHEMA_DEFINITIONS: tuple[SchemaDefinition, ...] = (
    ...
    SchemaDefinition(
        "session_projection", SessionProjection, "memory/sessions/{session_id}/projection/projection.json"
    ),
    ...
    SchemaDefinition("command_envelope", CommandEnvelope, "public command dispatch envelope"),
    ...
    SchemaDefinition("prose_artifact_payload", ProseArtifactPayload, "strict inter-agent prose envelope"),
    SchemaDefinition("segment_selection", SegmentSelection, "stable Markdown block selection"),
    SchemaDefinition("segment_patch", SegmentPatch, "scoped replacement generated by Revision Task"),
    SchemaDefinition(
        "revision_session", RevisionSession, "memory/revision_sessions/{revision_session_id}/session.json"
    ),
    ...
    SchemaDefinition(
        "canon_apply_log", CanonApplyLog, "memory/canon/applied_proposals/{canon_apply_id}/apply_log.json"
    ),
    ...
    SchemaDefinition(
        "session_rewrite_events", SessionRewriteEvents, "memory/sessions/{session_id}/rewrite_events.json"
    ),
    SchemaDefinition("memory_change_impact", MemoryChangeImpact, "embedded setting-change impact analysis"),
    SchemaDefinition(
        "memory_change_followup_action", MemoryChangeFollowupAction, "embedded setting-change follow-up action"
    ),
    SchemaDefinition(
...
MODEL_OUTPUT_SCHEMA_DEFINITIONS: tuple[SchemaDefinition, ...] = (
    SchemaDefinition("ProseArtifactPayload", ProseArtifactPayload, "structured prose inter-agent output"),
    ...
    SchemaDefinition("SegmentPatch", SegmentPatch, "Revision Agent structured patch output"),
)


def schema_payloads() -> dict[str, dict[str, object]]:
    return {definition.name: definition.model.model_json_schema() for definition in SCHEMA_DEFINITIONS}


def model_output_schema_payloads() -> dict[str, dict[str, object]]:
    return {definition.name: definition.model.model_json_schema() for definition in MODEL_OUTPUT_SCHEMA_DEFINITIONS}
...
def _schema_skeleton(node: object, defs: dict[str, object], *, depth: int = 0) -> object:
    ...
    if node_type == "object" or "properties" in node:
        ...
        if not isinstance(props, dict):
            return {}
        return {str(key): _schema_skeleton(value, defs, depth=depth + 1) for key, value in list(props.items())[:24]}
    if node_type == "array":
