...
def test_revise_chapter_creates_immutable_candidate(tmp_path: Path) -> None:
    ...
    result = revise_chapter(
        ...
        MockProvider(fake_response=_revision_payload("修订后的正文保留广播与车票，并让语气更克制。")),
    )
...
def test_revise_chapter_missing_style_guide_injects_chinese_fallback(tmp_path: Path) -> None:
    ...
    provider = MockProvider(fake_response=_revision_payload("修订后的正文保留广播与车票，并让语气更克制。"))
...
def test_revise_chapter_from_audit_records_immutable_candidate(tmp_path: Path) -> None:
    ...
    result = revise_chapter(
        ...
        MockProvider(fake_response=_revision_payload("根据审核意见修订后的正文。")),
    )
...
def test_revision_log_appends_distinct_immutable_candidates(tmp_path: Path) -> None:
    root = _workspace_with_generated_chapter(tmp_path)
    provider = MockProvider(fake_response=_revision_payload("修订后的正文。"))
...
def test_revise_chapter_search_context_protects_hidden_truth(tmp_path: Path) -> None:
    root = _workspace_with_generated_chapter(tmp_path)
    provider = MockProvider(fake_response=_revision_payload("修订后的正文仍保留广播与车票，只让林澈意识到雨夜还有未解之事。"))
...
def _revision_payload(body: str) -> str:
    return mock_prose_artifact_json(
        artifact_kind=ProseArtifactKind.CHAPTER_REVISION,
        chapter_number=1,
        body_markdown=body,
        source_artifact_refs=("memory/chapters/001/polished.md",),
        change_summary="测试生成修订候选稿。",
    )
