...
def test_revision_session_cli_exposes_blocks_run_and_accept(tmp_path: Path) -> None:
    ...
    assert _run_cli(
        ["revision-session", "run", revision_session_id, "--path", str(root), "--provider", "mock"]
    )[0] == 0
...
def _accepted_workspace(tmp_path: Path, *, chapters: str) -> Path:
    ...
    assert _run_cli(["session", "start", "连续创作", "--path", str(root), "--chapters", chapters, "--provider", "mock"])[0] == 0
